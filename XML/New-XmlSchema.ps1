function New-XmlSchema
{
	<#
		...
		.PARAMETER SchemaFilePath
			This is a mandatory parameter that you must use to specify the future file path of the schema file. The directory
			for this file path must exist. The file can or cannot exist. How the function handles this depends on if the
			-Force parameter is used. The file extension must be XSD.
	...
	process {
		try
		{
			...
			$XmlFilePath = New-Object System.IO.FileStream($SchemaFilePath, [IO.FileMode]::CreateNew)
			
			# Create XmlTextWriter with UTF8 Encoding to write to file
			$xwriter = New-Object System.Xml.XmlTextWriter($XmlFilePath, [Text.Encoding]::UTF8)
